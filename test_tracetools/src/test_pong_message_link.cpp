...
public:
  ...
  : Node(NODE_NAME, options), do_only_one_(do_only_one)
  {
    ...
    // Annotate message links for tracing:
    // Publisher publishes asynchronously using both subscriptions.
    std::vector<const void *> link_subs;
    link_subs.push_back(static_cast<const void *>(sub1_->get_subscription_handle().get()));
...
private:
  ...
  void callback2(const std_msgs::msg::String::ConstSharedPtr msg)
  {
    ...
    next_msg->data = "some random pong string including both: " +
